...
const confirmPasswordOfInput = document.querySelector(
  '.js-form-confirm-password'
);
const currentPasswordOfInput = document.querySelector(
  '.js-form-current-password'
);
const formElements = [
  passwordOfInput,
  confirmPasswordOfInput,
  currentPasswordOfInput,
];
const errorOfConfirmPassword = document.querySelector(
  '[data-name="confirm-password-error"]'
);
const eyeIcons = document.querySelectorAll('.js-eye-icon');
const submitButton = document.querySelector('.js-submit-button');

const getErrorElement = (element) =>
  element.parentElement.querySelector('.js-error');
const getInputElement = (element) =>
  element.parentElement.querySelector('input');
const isMatchValue = (input, confirmInput) =>
  input.value === confirmInput.value;
const isMatchPassword = (userData) =>
  currentPasswordOfInput.value === userData.password;
const hasInvalidClass = () =>
  formElements.some((element) => element.classList.contains('invalid'));
const confirmIfCanSubmit = () =>
  (submitButton.disabled = !isMatchValue(
    passwordOfInput,
    confirmPasswordOfInput
  ));
...
formElements.forEach((element) => {
  ...
  element.addEventListener('blur', (e) => {
    if (
      e.relatedTarget === element.parentElement.querySelector('.js-eye-icon')
    ) {
      if (
        element.value &&
        getErrorElement(element).textContent === '入力してください'
      )
        getErrorElement(element).textContent = '';
      return;
    }
    checkFormValidityInBlur(submitButton, element);

    if (passwordOfInput.value && confirmPasswordOfInput.value)
      errorOfConfirmPassword.textContent = isMatchValue(
        passwordOfInput,
        confirmPasswordOfInput
      )
        ? ''
        : '上記のPasswordと異なります。もう一度入力してください。';
    if (formElements.some((element) => element.classList.contains('invalid')))
      return;
...
submitButton.addEventListener('click', () => {
  ...
  if (!isMatchPassword(registeredData)) {
    document.querySelector('[data-name="current-password-error"]').textContent =
      'パスワードが一致しません';
