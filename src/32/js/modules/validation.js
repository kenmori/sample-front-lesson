...
export const showErrorMessage = (target) =>
  (target.nextElementSibling.textContent =
    validationOptions[target.name].errorMessage);
const removeErrorMessage = (target) =>
  (target.nextElementSibling.textContent = '');
export const isEmptyOfInput = (target) => target.value.trim() === '';
const isValidFormInput = (target) =>
  validationOptions[target.name].isValid(target);

export const confirmIfCanSubmit = (element, invalidItems) =>
  (element.disabled = invalidItems.length > 0);
