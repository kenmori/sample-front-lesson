...
const fetchImgData = async () => {
  return new Promise((resolve) => {
    setTimeout(
      () =>
        resolve(
          fetchData('https://mocki.io/v1/82e4e264-5a2d-4ebd-9814-7e63e47fb80b')
        ),
      3000
    );
...
const renderListOfImg = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    const li = createElementWithClassName(
      'li',
      'slideshow__pict-item js-slideshow-item'
    );
    const img = document.createElement('img');
...
const renderListOfPagination = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    const li = createElementWithClassName(
      'li',
      'pagination__item js-pagination-item'
    );
    const button = createElementWithClassName(
      'li',
      'pagination__btn js-pagination-btn'
    );
  ...
  pagination
    .insertAdjacentElement('afterbegin', paginationList)
