...
const isNewArrival = (date) => {
  ...
  const periodFromSubmission = differenceInCalendarDays(
    new Date(today),
...
const createTabNav = (values) => {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const button = createElementWithClassName(
      'button',
      'tab__nav-button js-tabNavButton'
    );
...
const appendArticlesTitleFragment = (values) => {
  ...
  for (let i = 0; i < articleTitles.length; i++) {
    ...
    isNewArrival(articleDate[i]) &&
      li.insertAdjacentElement('beforeend', createNewIcon());
  }
  return fragment;
};
...
const createCommentInfo = (values) => {
  const fragment = document.createDocumentFragment();
  const commentIconWrapper = createElementWithClassName(
    'div',
    'tab__contents-icon'
  );
  ...
  fragment
    .appendChild(commentIconWrapper)
    .insertAdjacentElement('afterend', commentLength);
  return fragment;
};
...
const createArticleContents = (data) => {
  ...
  //記事データの数だけulを作成
  for (let i = 0; i < values.length; i++) {
    const tabContents = createElementWithClassName(
      'div',
      'tab__contents js-tabContents'
    );
    const tabContentsInner = createElementWithClassName(
      'div',
      'tab__contents-inner'
    );
    ...
    tabContainer
      ...
      .appendChild(tabContentsInner)
      .appendChild(ul)
      .appendChild(articleTitlesFragment);
