...
const errorOfConfirmEmail = document.querySelector(
  '[data-name="confirm-email-error"]'
);
const errorOfPassword = document.querySelector(
...
const isMatchPassword = (userData) =>
  passwordOfInput.value === userData.password;
const hasInvalidClass = () =>
  formElements.some((element) => element.classList.contains('invalid'));
const confirmIfCanSubmit = () =>
  (submitButton.disabled = !isMatchValue(emailOfInput, confirmEmailOfInput));
...
formElements.forEach((element) => {
  ...
  element.addEventListener('blur', (e) => {
    ...
    if (emailOfInput.value && confirmEmailOfInput.value)
      errorOfConfirmEmail.textContent = isMatchValue(
        emailOfInput,
        confirmEmailOfInput
      )
        ? ''
